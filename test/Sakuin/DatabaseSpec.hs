  ...
  testGroup
    ...
    [ testCase "stores indexed paths by hash and replaces duplicates" $ do
        ...
        let storePath = StorePath "/nix/store" "hash" "example"
            ...
            initial = IndexedStorePath (WithOrigin entryOrigin storePath) (Directory Map.empty)
            replacement = IndexedStorePath (WithOrigin entryOrigin storePath) (Regular 42 False)
